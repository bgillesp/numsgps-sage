...
if not gap('LoadPackage("numericalsgps");'):
    raise ImportError('Error: unable to load gap package "numericalsgps"')
...
class Quasipolynomial:
	...
	def __call__(self, val):
		return sum([self.coeffs[i][val % self.period]*(val^i) for i in xrange(0, self.degree + 1)])
	...
	def FrobeniusNumbers(self, kmax, usezeros = False):
		...
		ret = [0 for i in xrange(0, kmax + 1)] if usezeros else {1:0}
		i = 0
		while exceedcount < m:
			i = i + 1
	...
	def FromCoefficients(vals, per, d, leadingcoeff = None):
		...
		ret = [[0 for j in xrange(0, per-1 + 1)] for i in xrange(0, d + 1)]
		for i in xrange(0, per-1 + 1):
			A = Matrix([[(l - 1 - (k2*per + i))^k1 for k1 in xrange(0, d + 1)] for k2 in xrange(0, d + 1)])

			b = vector([vals[l - 1 - (k*per + i)] for k in xrange(0, d + 1)])
			if leadingcoeff != None:
				b = b - vector([leadingcoeff[(l - 1 - i) % per]*((l - 1 - (k2*per + i))^(d+1)) for k2 in xrange(0, d + 1)])
...
class NumericalSemigroup:
	...
	def IsSymmetric(self):
		return self.FrobeniusNumber() % 2 == 1 and len([i for i in xrange(1, self.FrobeniusNumber() + 1) if i not in self]) == (self.FrobeniusNumber() + 1)/2
	...
	def FrobeniusNumbers(self,maxk):
		...
		ret = {i:0 for i in xrange(0, maxk + 1)}
	...
	def DeltaSetRingBufferUpToElement(self, nmax):
		...
		lensetringbuffer = [[] for i in xrange(1, gk + 1)]
		lensetringbuffer[0] = [0]

		deltasetringbuffer = [[] for i in xrange(1, totalperiod + 1)]
	...
	# 	mvals = [i for i in xrange(1, nmax + 1) if i in self and i not in self.__catenaries]
	...
	def SetOfElasticities(self, elastuppererror):
		...
		ret = []
		for i in xrange(1 , noisebound-1 + 1):
	...
	def OmegaCumulative(self, maxval, should_use_store_loc = True):
		inlist = xrange(-self.FrobeniusNumber() , maxval + 1)
	...
	def OmegaQuasilinearityDissonance(self):
		...
		return max([i for i in xrange(-self.FrobeniusNumber() , bound + 1) if self.__omegas[i] + 1 != self.__omegas[i+self.gens[0]]])
	...
	def SemigroupsWithGeneratorBounds(emb,nmax):
		values = [[i] for i in xrange(emb, nmax + 1)]
		totest = values
...
MonotonePrimitiveElementsOfNumericalSemigroup:=function(s)
	...
	n:=Length(l);
	ones:=List(xrange(1, n + 1),_->1);
	mat:=[];
	mat[1]:=Concatenation(l,-l,[0]);
	mat[2]:=Concatenation(ones,-ones,[1]);
	facs:=Set(4ti2Interface_hilbert_equalities_in_positive_orthant(mat),m->m{xrange(1, n + 1)});
...
MonotonePrimitiveElementsOfNumericalSemigroup_graver:=function(s)
    ...
    return Set(matrix, x->x{xrange(1, n + 1)}*l);
...
#print gap('OmegaPrimalityOfElementListInNumericalSemigroup(xrange(0, 1001),NumericalSemigroup("generators", [6,9,20]));')
